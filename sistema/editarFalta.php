...
if (!empty($_POST)) {
    if (empty($_POST['idPersonal']) || empty($_POST['fechaFalta']) || empty($_POST['tipoFalta']) || empty($_POST['observaciones'])) {
        ...
        $query = $con->prepare("UPDATE faltas SET idPersonal=:idPersonal, fecha=:fechaFalta, idTipoFalta=:tipoFalta, observaciones=:observaciones WHERE idFalta=$idFalta");
        ...
        $query->execute();
        if ($query) {
            $alert = '<p class="msg_save">Falta actualizada correctamente</p>';
        } else {
            $alert = '<p class="msg_error">Error al actualizar la falta</p>';
        }
    }
}
        ...
        <div class="form-Registro2">
            ...
            <form action="" method="POST">
                ...
                <select name="idPersonal" id="idPersonal">
                    ...
                    <?php
                    $query = $con->query("SELECT*FROM personal Where idPersonal=$resultado[idPersonal]");
                ...
                <input type="date" name="fechaFalta" id="fechaFalta" value="<?php echo $resultado['fecha'] ?>">
                ...
                <select name="tipoFalta" id="tipoFalta">
                    ...
                    <?php }else{?>
                        ...
                        <option value="2"selected>Retardo</option>
                    <?php }?>
                </select>
